	...
	try:
		if X>=0 and Y>=0:
			if Rebotes==0:
				...
				#if Rebotes<3:
				#	pixelesResultado[X,Y]=ROJO
				#else:
				#	pixelesResultado[X,Y]=VERDE
				X+=1*math.sin(Angulo)
				Y+=1*math.cos(Angulo)
	...
	try:
		if listaTiempos!=[]:
			for Tiempos in listaTiempos:
				...
				pixelesResultado[XFinal, YFinal]=CalcularColor(Tiempos)
	except (IndexError):
		pass
...
imagenResultado=imagenResultado.convert("RGB")
